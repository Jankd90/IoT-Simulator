...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # import network
    gen_net = Generator(seq_len=150, channels=6, num_classes_1=2, num_classes_2=4, latent_dim=100, data_embed_dim=10, 
                        label_embed_dim=10 ,depth=3, num_heads=5, 
                        forward_drop_rate=0.5, attn_drop_rate=0.5)
    
    print(gen_net)
    dis_net = Discriminator(in_channels=6, patch_size=1, data_emb_size=50, label_emb_size=10, seq_length = 150, depth=3, n_classes_1=2, n_classes_2=4)
    print(dis_net)
    ...
    X_train, X_test, y1_train_agitation, y1_test_agitation, y2_train_activity, y2_test_activity, agitation_label_mapping, activity_label_mapping = data_processor.load_and_process_data()

    np.savez_compressed('X_test.npz', X_test)
    np.savez_compressed('y1_test_agitation.npz', y1_test_agitation)
    np.savez_compressed('y2_test_activity.npz', y2_test_activity)

    train_set = mitbih_train(X_train, y1_train_agitation, y2_train_activity)
...
def gen_plot(gen_net, epoch):
    ...
    for i in range(10):
        ...
        fake_noise = torch.FloatTensor(np.random.normal(0, 1, (1, 100)))
        fake_label_1 = torch.randint(0, 2, (1,))
        fake_label_2 = torch.randint(0, 4, (1,))
        fake_sigs = gen_net(fake_noise, fake_label_1, fake_label_2).to('cpu').detach().numpy()
        
        synthetic_data.append(fake_sigs)
        # synthetic_labels.append(fake_label)
        synthetic_labels_1.append(fake_label_1)
        synthetic_labels_2.append(fake_label_2)
    ...
    for i in range(2):
        for j in range(5):
            ...
            axs[i, j].title.set_text(f"Label1: {synthetic_labels_1[i*5+j].item()}, Label2: {synthetic_labels_2[i*5+j].item()}")
    buf = io.BytesIO()
