...
def train(args, gen_net: nn.Module, dis_net: nn.Module, gen_optimizer, dis_optimizer, gen_avg_param, train_loader,
    ...
    for iter_idx, (real_imgs, real_img_labels_1, real_img_labels_2) in enumerate(tqdm(train_loader)):
...
#         real_img_labels = real_img_labels.type(torch.IntTensor)
        real_img_labels_1 = real_img_labels_1.type(torch.LongTensor)
        real_img_labels_1 = real_img_labels_1.cuda(args.gpu, non_blocking=True)

        real_img_labels_2 = real_img_labels_2.type(torch.LongTensor)
        real_img_labels_2 = real_img_labels_2.cuda(args.gpu, non_blocking=True)
        ...
        fake_img_labels_1 = torch.randint(0, 2, (real_imgs.shape[0],)).cuda(args.gpu, non_blocking=True)
        fake_img_labels_2 = torch.randint(0, 4, (real_imgs.shape[0],)).cuda(args.gpu, non_blocking=True)
        ...
        dis_net.zero_grad()
        r_out_adv, r_out_cls_1, r_out_cls_2 = dis_net(real_imgs)
        fake_imgs = gen_net(noise, fake_img_labels_1, fake_img_labels_2)
        
        assert fake_imgs.size() == real_imgs.size(), f"fake_imgs.size(): {fake_imgs.size()} real_imgs.size(): {real_imgs.size()}"

        f_out_adv, f_out_cls_1, f_out_cls_1 = dis_net(fake_imgs)
        ...
        d_cls_loss_1 = cls_criterion(r_out_cls_1, real_img_labels_1)
        d_cls_loss_2 = cls_criterion(r_out_cls_2, real_img_labels_2)
        d_cls_loss = d_cls_loss_1 + d_cls_loss_2
        ...
        gen_imgs = gen_net(noise, fake_img_labels_1, fake_img_labels_2)
        g_out_adv, g_out_cls_1, g_out_cls_2 = dis_net(gen_imgs)

        g_adv_loss = -torch.mean(g_out_adv)

        g_cls_loss_1 = cls_criterion(g_out_cls_1, fake_img_labels_1)  
        g_cls_loss_2 = cls_criterion(g_out_cls_2, fake_img_labels_2)

        g_cls_loss = g_cls_loss_1 + g_cls_loss_2
