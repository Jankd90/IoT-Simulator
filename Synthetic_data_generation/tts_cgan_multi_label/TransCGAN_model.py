...
class Generator(nn.Module):
    def __init__(self, seq_len=150, channels=3, num_classes_1=9, num_classes_2=4, latent_dim=100, data_embed_dim=10, 
        ...
        self.l1 = nn.Linear(self.latent_dim + 2 * self.label_embed_dim, self.seq_len * self.data_embed_dim)
        self.label_embedding_1 = nn.Embedding(self.num_classes_1, self.label_embed_dim)
        self.label_embedding_2 = nn.Embedding(self.num_classes_2, self.label_embed_dim) 
    ...
    def forward(self, z, labels_1, labels_2):
        c_1 = self.label_embedding_1(labels_1)
        c_2 = self.label_embedding_2(labels_2)
...
class ClassificationHead(nn.Sequential):
    def __init__(self, emb_size=100, adv_classes=2, cls_classes_1=10, cls_classes_2=5):
        super().__init__()
        ...
        self.cls_head_1 = nn.Sequential(
            Reduce('b n e -> b e', reduction='mean'),
            nn.LayerNorm(emb_size),
            nn.Linear(emb_size, cls_classes_1)
        )
        self.cls_head_2 = nn.Sequential(
            Reduce('b n e -> b e', reduction='mean'),
            nn.LayerNorm(emb_size),
            nn.Linear(emb_size, cls_classes_2)
        )

    def forward(self, x):
        out_adv = self.adv_head(x)
        out_cls_1 = self.cls_head_1(x)
        out_cls_2 = self.cls_head_2(x)
        return out_adv, out_cls_1, out_cls_2
...
class Discriminator(nn.Sequential):
    def __init__(self, 
        ...
        super().__init__(
            ...
            ClassificationHead(data_emb_size, 1, n_classes_1, n_classes_2)
        )
